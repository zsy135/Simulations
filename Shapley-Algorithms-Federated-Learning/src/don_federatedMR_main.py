...
if __name__ == '__main__':
    ...
    history_shapley = []
    with open('../save/MRfed_OurMNIST_mlp_5_1_old.txt', 'r') as f:
    ...
    for epoch in tqdm(range(args.epochs)):
        ...
        while len(list_s) < 2 and i < len(list_kAndSita):
            ...
            i = i + 1
        while i < len(list_kAndSita)  and len(list_s_index) < m:  # 5为用户数量and if_add_to_s(i)
            if list_kAndSita[i].sita >= theta_th:
                list_s.append(list_kAndSita[i])
                list_s_index.append(i)
            i = i + 1
        ...
        list_s_index = [i + epoch for i in list_s_index]
    ...
    for value in values:
        ...
        decay=0.5
        if len(value)==1:
            aggregate_list.append((value[0])*decay)
        else:
