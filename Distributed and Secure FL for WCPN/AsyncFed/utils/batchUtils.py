...
def startAllNodesOnTerminal(maliciousNodes):
   ...
   for i in range(config.ClientNum):
      if i in maliciousNodes:
         proc = subprocess.Popen(["gnome-terminal","--","/bin/bash","-c","python3", "startWorker.py", str(i), 'true'])
      else:
         proc = subprocess.Popen(["gnome-terminal","--","/bin/bash","-c","python3", "startWorker.py", str(i)])
...
def startAllNodesOnFile(maliciousNodes):
   ...
   for i in range(config.ClientNum):
      if i in maliciousNodes:
         proc = subprocess.Popen(["python3", "startWorker.py",str(i),'true'],stdout=writers[i])
      else:
         proc = subprocess.Popen(["python3","startWorker.py",str(i)],stdout=writers[i])
      nodesProc.append(proc)
      print("------- Start Node{} -------".format(i))
      time.sleep(0.5)
...
def startAllNodesOnFileOnWindows(maliciousNodes):
   outFileNameTmpl = "./work/Node{}_OutPut.txt"
   writers = getWriters(config.ClientNum,outFileNameTmpl)
   nodesProc = []
